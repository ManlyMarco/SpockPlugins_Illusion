...
{
    private static Dictionary<string, string> HCVoiceMap = new()
    {
    ...
    private static H.HScene HSceneInstance;
    private static HC.Scene.HomeScene HomeSceneInstance;

    private static class Hooks
    {
        private static string getSubtitle(string audioFile)
        {
                    ...
                    foreach (var mode in list.Values)
                        foreach (var sheet in mode.Values)
                            foreach (var kind in sheet.DicdicVoiceList)
                                ...
                                {
                                    if (voiceID.NameFile == audioFile) return voiceID.Word;
                                }
                }
            else if (HomeSceneInstance != null) return HCVoiceMap.GetValueSafe(audioFile);
        ...
        private static void PlayVoice(AudioSource audioSource, Manager.Voice.Loader loader, Il2CppSystem.Action<AudioSource> afterAction)
        {
            if (loader.Asset.IsNullOrEmpty())
                return;
            var audioFile = loader.Asset;
            var subtitle = getSubtitle(audioFile);
            Log.LogInfo($"Play voice({audioFile}): {subtitle}");
            if (!subtitle.IsNullOrEmpty()) SubtitlesCanvas.DisplaySubtitle(audioSource, subtitle);
        }

        [HarmonyPostfix]
        [HarmonyPatch(typeof(H.HScene), nameof(H.HScene.Start))]
        ...
        [HarmonyPostfix]
        [HarmonyPatch(typeof(HC.Scene.HomeScene), nameof(HC.Scene.HomeScene.Start))]
